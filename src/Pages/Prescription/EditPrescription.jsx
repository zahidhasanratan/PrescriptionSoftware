...
export default function EditPrescription() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      .catch(() =>
        Swal.fire("Error", "Failed to load data", "error").then(() =>
  ...
  const build = (field, val) => {
    ...
    allMeds.forEach((m) => {
      ...
      if (field === "strength")
        m.commonStrengths?.forEach((s) => set.add(s));
  ...
  const onKey = (e, field) => {
    ...
    if (e.key === "ArrowDown") {
      e.preventDefault();
      setHi((h) => ({ ...h, [field]: (h[field] + 1) % lst.length }));
    }
    if (e.key === "ArrowUp") {
      e.preventDefault();
      setHi((h) => ({
        ...h,
        [field]: (h[field] - 1 + lst.length) % lst.length,
      }));
    }
    if (e.key === "Enter") {
      e.preventDefault();
      selectOption(lst[hi[field]], field);
    }
  };
  ...
  const update = async () => {
    if (!meds.length)
      return Swal.fire("Missing", "Add at least one medicine", "warning");
    try {
      await axios.put(
        `https://prescription-ebon.vercel.app/api/prescriptions/${id}`,
        {
          patient,
          medicines: meds,
          notes,
    ...
    <div className="max-w-5xl mx-auto bg-white shadow p-6 rounded space-y-6">
      ...
      <div className="space-y-3">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          {["name", "type", "strength", "dosage", "advice", "duration"].map(
            (f) => (
      ...
      {meds.map((m, i) => (
        ...
        >
          <span>
            <strong>{m.name}</strong> ({m.type}) {m.strength} – {m.dosage} •{" "}
            {m.duration} • {m.advice}
          </span>
