...
// ← Change this to your real backend base URL (no trailing slash)
...
export const WritePrescription = () => {
  ...
  const savePrescription = async () => {
    ...
    // prescriptionNumber e.g. "GE/1", "GE/2", ...
    const catCode = patientForm.category.slice(0,2).toUpperCase();
    const allRx = await axios.get(`${API_BASE}/prescriptions`).then(r=>r.data);
    const serial = allRx.filter(r=>r.prescriptionNumber?.startsWith(catCode)).length + 1;
    const prescriptionNumber = `${catCode}/${serial}`;
    ...
    <div className="bg-white p-6 rounded-xl shadow-md max-w-5xl mx-auto space-y-6">
      {combinedReports.length>0 && (
        <div className="alert alert-info flex items-center gap-3">
          <Paperclip size={18}/>
          <span>{combinedReports.length} report{combinedReports.length>1?"s":""} attached</span>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        ...
        <textarea
          ...
          value={notes.tests}
          onChange={e=>setNotes(n=>({...n,tests:e.target.value}))}
        />
        <textarea
          className="textarea textarea-bordered"
          rows={2}
          ...
          onChange={e=>setNotes(n=>({...n,generalAdvice:e.target.value}))}
        />
      </div>
